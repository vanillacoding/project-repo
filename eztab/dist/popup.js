...
;// CONCATENATED MODULE: ./src/utils/getClosestTargetBySelector.js
...
/* harmony default export */ const utils_getClosestTargetBySelector = (getClosestTargetBySelector);
...
class Controller {
  ...
  async init() {
    ...
    this.view.$navigation.addEventListener("click", ({ target }) => {
      const $navLink = utils_getClosestTargetBySelector(target, ".nav-link");
  ...
  addSearchEvent() {
    this.view.$search.addEventListener(
      "input",
      lodash_default().debounce(({ target }) => {
...
/* harmony default export */ const src_controller = (Controller);
...
/* harmony default export */ const src_model = (Model);

;// CONCATENATED MODULE: ./src/utils/convertHTMLSpecialCharacters.js
const convertHTMLSpecialCharacters = (str) => {
  return str.replace(/</g, "&lt;").replace(/>/g, "&gt;");
};

/* harmony default export */ const utils_convertHTMLSpecialCharacters = (convertHTMLSpecialCharacters);

;// CONCATENATED MODULE: ./src/templates/groupTabEntry.js
...
const generateGroupTabEntryTemplate = (tab) => {
  ...
  <div class="tab-entry">
    <button class="tab-title-button">
      <h3 class="tab-title">${utils_convertHTMLSpecialCharacters(tab.title)}</h3>
    </button>
...
/* harmony default export */ const groupTabEntry = (generateGroupTabEntryTemplate);

;// CONCATENATED MODULE: ./src/templates/tabGroup.js
...
const generateTabGroupTemplate = (tabs, groupName) => {
  ...
  <div class="tab-group" data-group-name=${encodeURIComponent(groupName)}>
    ...
    <div class="expansion">
      <div class="tab-list">
        ${tabs.map((tab) => groupTabEntry(tab)).join("")}
      </div>
    </div>
  </div>
  `;
...
/* harmony default export */ const tabGroup = (generateTabGroupTemplate);

;// CONCATENATED MODULE: ./src/templates/tabEntry.js
...
/* harmony default export */ const tabEntry = (generateTabEntryTemplate);

;// CONCATENATED MODULE: ./src/templates/tabList.js
...
const generateTabListTemplate = ({ id, tabs, isCurrent }) => {
  ...
  <div class="window" data-window-id=${id}>
    ...
    <div class="tab-list">
      ${tabs.map((tab) => tabEntry(tab)).join("")}
    </div>
  </div>
  `;
};

/* harmony default export */ const tabList = (generateTabListTemplate);

;// CONCATENATED MODULE: ./src/utils/fadeOut.js
const fadeOut = (el) => {
  el.style.opacity = 1;
...
/* harmony default export */ const utils_fadeOut = (fadeOut);

;// CONCATENATED MODULE: ./src/utils/parseHTML.js
...
/* harmony default export */ const utils_parseHTML = (parseHTML);
...
class View {
    ...
    switch (name) {
      case "Current Tabs": {
        const template = data.payload.windows.reduce((acc, curr) => {
          return acc + tabList(curr);
        }, "");

        this.$currentTabs.appendChild(utils_parseHTML(template));
      ...
      case "Tab Groups": {
        ...
        const template = data.payload.groups.reduce((acc, curr) => {
          return acc + tabGroup(curr.tabs, curr.groupName);
        }, "");

        this.$tabGroups.textContent = "";
        this.$tabGroups.appendChild(utils_parseHTML(template));
  ...
  getWindowId($elem) {
    return Number(
      utils_getClosestTargetBySelector($elem, ".window").dataset.windowId
    );
  }
  ...
  removeTab($elem, callback) {
    ...
    utils_getClosestTargetBySelector($elem, ".tab-entry").remove();
    callback(tabId);
  }

  removeGroupTab($elem, callback) {
    const $tabList = utils_getClosestTargetBySelector($elem, ".tab-list").children;
    const $tabGroup = utils_getClosestTargetBySelector($elem, ".tab-group");
  ...
  createToast(text) {
    ...
    setTimeout(() => utils_fadeOut($toast), 500);
  }
}

/* harmony default export */ const src_view = (View);
...
const init = () => {
  const model = new src_model();
  const view = new src_view();
  const controller = new src_controller(model, view);
  controller.init();
};
